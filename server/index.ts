...
const SHOPPING_LIST_STATE_PATH = path.join(
  OUTPUT_DIR,
  'shopping-list-state.json'
);
...
function withSerial(
  ...
  return async (req, res) => {
    if (processing) {
      res
        .status(409)
        .json({ success: false, error: 'Already processing a request' });
      return;
    }
    processing = true;
    try {
...
function writeFlippCache(flyerId: number, result: ExtractionResult) {
  ...
  writeJsonAtomic(path.join(FLIPP_CACHE_DIR, `${flyerId}.json`), {
    ...result,
    _cachedAt: Date.now(),
  });
}
...
function clearShoppingListState() {
  const existing = readJsonOrNull<{ extraItems?: unknown }>(
...
app.use(
  '/api/circular/flipp/fetch',
  rateLimit({ windowMs: 60_000, limit: 20 })
);
app.use(
  '/api/extra-items/estimate',
...
app.put(
  ...
  asyncRoute((req, res) => {
    ...
    if (key.length > MAX_KEY_LEN) {
      throw new HttpError(
        400,
        `geminiApiKey must be ${MAX_KEY_LEN} chars or fewer`
      );
    }
    saveGeminiKey(key);
  ...
  asyncRoute((req, res) => {
    ...
    for (const k of body.checkedKeys) {
      ...
      if (k.length > MAX_KEY_LEN) {
        throw new HttpError(
          400,
          `checkedKeys entries must be ${MAX_KEY_LEN} chars or fewer`
        );
      }
      seen.add(k);
      if (seen.size > MAX_CHECKED_KEYS) {
        throw new HttpError(
          400,
          `checkedKeys can have at most ${MAX_CHECKED_KEYS} entries`
        );
      }
    }
    ...
    if (body.extraItems !== undefined) {
      ...
      if (body.extraItems.length > MAX_EXTRA_ITEMS) {
        throw new HttpError(
          400,
          `extraItems can have at most ${MAX_EXTRA_ITEMS} entries`
        );
      }
    ...
    const persistedExtras =
      extraItems ?? normalizeExtraItems(existing?.extraItems);
    writeJsonAtomic(SHOPPING_LIST_STATE_PATH, {
      planId: body.planId,
      checkedKeys: [...seen],
...
app.get('/api/meal-plan', (_req, res) => {
  const data = readJsonOrNull<MealPlanResult & Record<string, unknown>>(
    MEAL_PLAN_PATH
  );
  if (!data) {
    res.json({ exists: false });
  ...
  const planId =
    typeof data.planId === 'string' && data.planId ? data.planId : null;
...
app.post(
  ...
  asyncRoute(
    withSerial(async (_req, res) => {
      const extraction = readJsonOrNull<ExtractionResult & unknown[]>(
        EXTRACTION_PATH
      );
      if (!extraction) {
        throw new HttpError(
          400,
          'No circular extracted yet. Upload a circular first.'
        );
      }
...
app.post(
  '/api/meal-plan/generate-no-circular',
  asyncRoute(
    withSerial(async (req, res) => {
...
app.post(
  ...
  asyncRoute(
    withSerial(async (req, res) => {
      ...
      if (
        typeof day !== 'string' ||
        !VALID_DAYS_OF_WEEK.has(day.toLowerCase())
      ) {
      ...
      const mealsForDay =
        preferences.mealsByDay[day.toLowerCase() as DayOfWeek] ?? [];
      if (!mealsForDay.includes(slotKey)) {
        throw new HttpError(
          400,
          `${mealType} is not enabled for ${day} in current preferences`
        );
      }
      ...
      const result = await generateMealSwap(
        plan,
        day,
        slotKey,
        saleItems,
      ...
      // Retained rows keep the unit they were generated with; re-join it for any
      // ingredient the swap newly introduced. Issue #121.
      attachSaleUnits(mergedShoppingList, saleItems);
  ...
  asyncRoute(
    withSerial(async (req, res) => {
      ...
      if (!from || typeof from !== 'object' || !to || typeof to !== 'object') {
        throw new HttpError(
          400,
          'from and to must be objects with day and mealType'
        );
      }
      ...
      if (
        ...
        !VALID_DAYS_OF_WEEK.has(fromDay.toLowerCase())
      ) {
        throw new HttpError(400, 'from.day must be a valid day of the week');
      }
      if (
        typeof toDay !== 'string' ||
        !VALID_DAYS_OF_WEEK.has(toDay.toLowerCase())
      ) {
      ...
      if (typeof fromType !== 'string' || !VALID_MEAL_TYPES.has(fromType)) {
        throw new HttpError(
          400,
          "from.mealType must be 'breakfast', 'lunch', or 'dinner'"
        );
      }
      if (typeof toType !== 'string' || !VALID_MEAL_TYPES.has(toType)) {
        throw new HttpError(
          400,
          "to.mealType must be 'breakfast', 'lunch', or 'dinner'"
        );
      }
      ...
      try {
        ...
        if (fs.existsSync(tmpPath)) {
          try {
            ...
            console.warn(
              `[upload] failed to clean up tmp file ${tmpPath}:`,
              err
            );
          }
        }
  ...
  asyncRoute(
    withSerial(async (req, res) => {
      ...
      if (!Number.isFinite(flyerId) || !merchantName) {
        throw new HttpError(
          400,
          'flyerId (number) and merchantName (string) are required'
        );
      }
      ...
      try {
        ...
        const result = await runScanAndPlan(extraction);
        if (merchantId !== null)
          saveCircularPrefs({ lastMerchantId: merchantId });
        res.json({ success: true, ...result });
      } finally {
        scanProgress = { stage: 'idle' };
      }
